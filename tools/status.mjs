...
// builder completion records: docs/builds/<module>_r<n>.json
const bdir = 'docs/builds';
const builds = fs.existsSync(bdir) ? fs.readdirSync(bdir).filter((f) => /^[a-z]+_r\d+\.json$/.test(f)) : [];
for (const f of builds) {
  const m = f.match(/^([a-z]+)_r(\d+)\.json$/);
  const rec = status.modules[m[1]]; if (!rec) continue;
  rec.builtRound = Math.max(rec.builtRound || 0, +m[2]);
}
for (const [, rec] of Object.entries(status.modules)) {
