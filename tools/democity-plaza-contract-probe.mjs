...
const baseline = process.env.SIM_BASELINE_URL || 'http://127.0.0.1:5189';
const candidate = process.env.SIM_CANDIDATE_URL || 'http://127.0.0.1:5190';
const out = process.env.SIM_DEMOCITY_PLAZA_OUT || 'shots/democity/plaza-contract.json';
...
async function capture(base, seed) {
  ...
  const data=await page.evaluate(()=>{
    ...
    const plaza=ordered(w.services.items).filter(v=>v.kind==='plaza').map(v=>({id:v.id,x:v.x,z:v.z,heading:v.heading,footprint:v.footprint}));
    return {roads:JSON.stringify(roads),lots:JSON.stringify(lots),buildings:JSON.stringify(buildings),services:JSON.stringify(services),serviceRoundTripExact:JSON.stringify(before)===JSON.stringify(after),park,plaza,stats:api.democity.stats(),economy:{population:w.economy.population,jobs:w.economy.jobs,money:w.economy.money,net:w.economy.net,happiness:w.economy.happiness},errors:s.errors.slice()};
  });
  await page.close();
  return {seed,hashes:{roads:hash(data.roads),lots:hash(data.lots),buildings:hash(data.buildings),services:hash(data.services)},counts:{roads:JSON.parse(data.roads).length,lots:JSON.parse(data.lots).length,buildings:JSON.parse(data.buildings).length,services:JSON.parse(data.services).length},park:data.park,plaza:data.plaza,serviceRoundTripExact:data.serviceRoundTripExact,economy:data.economy,engineErrors:data.errors,browserErrors:errors};
}
...
const ok=runs.every(r=>Object.values(r.unchanged).every(Boolean)&&r.candidateDeterministic&&r.candidateServiceRoundTrip&&r.candidate.park.length===1&&r.candidate.plaza.length===1&&r.candidate.counts.services===r.baseline.counts.services+1&&r.candidate.engineErrors.length===0&&r.candidate.browserErrors.length===0);
