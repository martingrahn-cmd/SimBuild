#!/usr/bin/env node
// R9i: disposable actual road-owner/frontage test for one R8y-ranked missing link.
...
const root = process.env.OUT_DIR || 'shots/democity/r9i-actual-frontage';
...
const expose = "globalThis.__R9I_GRID=[...nodes.values()].map(n=>({...n}));globalThis.__R9I_EDGE_ID=-1;P.nodes=nodes;return P;";
const insert = "const r9ia=nodes.get('-8,-11'),r9ib=nodes.get('-7,-11');globalThis.__R9I_EDGE_ID=r9ia&&r9ib?add(r9ia.id,r9ib.id,'alley'):-1;globalThis.__R9I_GRID=[...nodes.values()].map(n=>({...n}));P.nodes=nodes;return P;";
const variants = {
...
const result = {
  ...
  candidate: { grid: '-8,-11->-7,-11', type: 'alley', rationale: 'Like-for-like follow-up to the R8y alley-width proxy and rejected R9g street; same endpoints, actual Roads/Zoning owners, contact risk measured rather than hidden' },
...
const run = async (variant, seed, repeat = false, directedTargetY = null) => {
  ...
  try {
    ...
    await page.waitForFunction(() => window.__sim?.ready === true && Array.isArray(globalThis.__R9I_GRID), null, { timeout: 240000 });
    ...
    const state = await page.evaluate(async ({ variant, seed }) => {
      ...
      const grid = globalThis.__R9I_GRID, a = grid.find(n => n.i === -8 && n.j === -11), b = grid.find(n => n.i === -7 && n.j === -11);
      const edgeId = globalThis.__R9I_EDGE_ID;
