#!/usr/bin/env node
// R9s: verify whole-game construction ownership plus complete road transaction restoration.
// Commit/redo leave replacement lots to Simulation; undo restores the exact retired building owner.
import { chromium } from 'playwright';
import fs from 'node:fs';

const base=process.env.SIM_URL||'http://127.0.0.1:5173';
const out=process.env.OUT_FILE||'shots/democity/r9s-road-complete-restore/construction-timing.json';
...
const result={method:'fresh whole-game all-alias worlds, paused; public road commit/undo/redo; commit/redo remain Simulation-paced while undo restores the exact retired owner',runs:[],errors:[],pass:false};
...
try{
 for(const seed of seeds){
  ...
  // Commit and redo leave invalidated replacement lots to ordinary Simulation. Undo must restore
  // the pre-transaction building and close every lot/building link without eager new construction.
  run.pass=run.pass&&after.counts.buildings===before.counts.buildings-1&&after.counts.freeLots>=1&&after.integrity.unbuiltLots.length===after.counts.freeLots&&undo.counts.buildings===before.counts.buildings&&undo.counts.lots===before.counts.lots&&undo.counts.freeLots===before.counts.freeLots&&undo.integrity.unbuiltLots.length===before.integrity.unbuiltLots.length&&redo.counts.buildings===after.counts.buildings&&redo.counts.lots===after.counts.lots&&redo.counts.freeLots===after.counts.freeLots&&redo.integrity.unbuiltLots.length===after.integrity.unbuiltLots.length&&undoResult===true&&redoResult===true&&after.money===before.money-96&&undo.money===before.money&&redo.money===after.money;
