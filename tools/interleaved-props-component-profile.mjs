#!/usr/bin/env node
// Repeated same-page A-B-A component masks with fixed reflection and headless-sync policy.
...
const reflection=process.env.REFLECTION!=='off',captureSync=process.env.CAPTURE_SYNC!=='off';
...
const result={method:'one warmed page per component; A0-B0-A1-B1-A2-B2-A3; each B compared with adjacent A mean',url:base,camera,time,frames,warmFrames,cycles,reflection,captureSync,components:[]};
...
try{
 ...
 result.pass=result.components.every(r=>r.inventory.objects>0&&r.summary.stableCycles===cycles&&!r.browserErrors.length&&r.blocks.every(b=>!b.errors.length));
 fs.mkdirSync(out.split('/').slice(0,-1).join('/')||'.',{recursive:true});fs.writeFileSync(out,JSON.stringify(result,null,2));console.log(JSON.stringify({pass:result.pass,camera,reflection,captureSync,rows:result.components.map(r=>({component:r.component,inventory:r.inventory,summary:r.summary,pairs:r.pairs}))},null,2));
