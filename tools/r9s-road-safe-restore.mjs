#!/usr/bin/env node
// R9s2: verify exact, owner-safe road history across all dependent owner snapshots.
...
const root=process.env.OUT_DIR||'shots/democity/r9s2-road-safe-restore';fs.mkdirSync(root,{recursive:true});
...
const sourcePaths=['src/modules/roads/index.js','src/modules/tools/tools.js','src/modules/zoning/index.js','src/modules/zoning/grid.js','src/modules/buildings/index.js','src/modules/simulation/index.js'];
...
const result={method:'fresh current-product worlds; disposable public alley transaction; exact owner restore required for undo and redo in two seeds',candidate:{key:'1,0:v',endpoints,type:'alley',productAdoption:false},sources:Object.fromEntries(sourcePaths.map(p=>[p,sha(fs.readFileSync(p))])),runs:[],comparisons:[],errors:[],pass:false};
...
const run=async(variant,seed,repeat=false)=>{
 ...
 try{
  ...
  await page.waitForFunction(()=>{
   const s=window.__sim,apis=s?.registry?.apis,names=['roads','zoning','buildings','simulation','services','transit','terrain'];
   return s?.ready===true&&apis&&names.every(name=>typeof apis[name]?.serialize==='function');
  },null,{timeout:240000});await settle(page,90);
  const before=await page.evaluate(snapshot);
  let transaction=null,after=before,undo=null,redo=null;
  if(variant==='candidate'){
   transaction=await page.evaluate(({a,b})=>{const s=window.__sim,t=s.registry.apis.tools;t.select('road',{type:'alley',mode:'straight',elevation:0,oneWay:false,snap:['magnet']});const first=t.pointer(a.x,a.z),click=t.click(0),second=t.pointer(b.x,b.z),draft=t.state(),commit=t.commit(),history=t.history();t.select(null);return{first,click,second,draft,commit,history};},endpoints);
  ...
  row.pass=!browserErrors.length&&!after.errors.length&&after.ready16&&after.integrity.bijective&&after.integrity.duplicateCells.length===0&&after.integrity.overlaps.length===0&&(variant==='baseline'||(transaction.draft.valid===true&&transaction.commit.ok===true&&undo?.result===true&&redo?.result===true&&undo.integrity.bijective&&redo.integrity.bijective&&!undo.errors.length&&!redo.errors.length));return row;
...
function snapshot(){
 ...
 const modules=s.stats().modules,lotIds=new Set(lots.map(l=>l.id)),buildingIds=new Set(buildings.items.map(b=>b.id));
 const orphanBuildings=buildings.items.filter(b=>!lotIds.has(b.lotId)).map(b=>({id:b.id,lotId:b.lotId,type:b.type,density:b.density}));
 const unbuiltLots=lots.filter(l=>l.buildingId==null||!buildingIds.has(l.buildingId)).map(l=>({id:l.id,buildingId:l.buildingId,stableKey:l.stableKey,type:l.type,density:l.density}));
 const mismatchedLinks=lots.filter(l=>l.buildingId!=null&&buildings.items.find(b=>b.id===l.buildingId)?.lotId!==l.id).map(l=>({id:l.id,buildingId:l.buildingId}));
 const integrity={oneBuildingPerLot:unbuiltLots.length===0,everyBuildingHasLot:orphanBuildings.length===0,orphanBuildings,unbuiltLots,mismatchedLinks,bijective:orphanBuildings.length===0&&unbuiltLots.length===0&&mismatchedLinks.length===0,duplicateCells,overlaps};
 return{counts:{roadNodes:roads.nodes.length,roadEdges:roads.edges.length,zonedCells:w.zones.cells.size,claimedCells:refs.size,lots:lots.length,buildings:w.buildings.items.size,services:w.services.items.size},roads,lots,zones,buildings,simulation,services,transit,terrainHash:(h>>>0).toString(16).padStart(8,'0'),terrainSamples:Array.from({length:21},(_,i)=>{const z=40+i*4;return{z,y:w.terrain.getHeight(80,z),slope:w.terrain.getSlope(80,z),road:w.roads.isRoad(80,z),surface:api.roads.surfaceHeightAt(80,z)}}),economy:{money:w.economy.money,population:w.economy.population,jobs:w.economy.jobs,net:w.economy.net,demand:clone(w.economy.demand),loans:clone(w.economy.loans),taxRate:clone(w.economy.taxRate)},versions:{roads:w.roads.version,zones:w.zones.version,buildings:w.buildings.version,terrain:w.terrain.version},components:components(),integrity,ready16:Object.values(modules).length===16&&Object.values(modules).every(m=>m.status==='ready'&&m.errors===0),errors:[...s.errors]};
}
...
result.pass=!result.errors.length&&result.runs.length===6&&result.runs.every(r=>r.pass)&&result.comparisons.length===2&&result.comparisons.every(c=>c.candidateRepeatExact&&c.undoExact&&c.undoSemantic&&c.redoExact&&c.redoSemantic&&c.countDelta.roadNodes===2&&c.countDelta.roadEdges===3&&c.removedEdgeIds.length===2&&c.addedEdgeIds.length===5&&c.unchangedExistingEdgeIdentity);
