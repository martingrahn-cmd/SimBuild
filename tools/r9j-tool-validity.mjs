#!/usr/bin/env node
// R9j: ask the shipped road tool whether the measured alley is a valid player action.
import { chromium } from 'playwright';
import fs from 'node:fs';
import path from 'node:path';

const base = process.env.SIM_URL || 'http://127.0.0.1:5173';
const root = process.env.OUT_DIR || 'shots/democity/r9j-legal-frontage-screen';
const chrome = process.env.SIM_CHROME || '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';
const endpoints = {
  1337: [{ x: -200, z: -520 }, { x: -120, z: -520 }],
  7: [{ x: -200, z: -528 }, { x: -120, z: -528 }],
};
...
result.pass = result.errors.length === 0 && result.runs.length === 2 && result.runs.every(r =>
  !r.browserErrors.length && !r.errors.length && r.draft.valid === true &&
