#!/usr/bin/env node
// R9g: disposable actual road-owner/frontage test for one R8y-ranked missing link.
...
const root = process.env.OUT_DIR || 'shots/democity/r9g-actual-frontage';
...
const expose = "globalThis.__R9G_GRID=[...nodes.values()].map(n=>({...n}));globalThis.__R9G_EDGE_ID=-1;P.nodes=nodes;return P;";
const insert = "const r9ga=nodes.get('-8,-11'),r9gb=nodes.get('-7,-11');globalThis.__R9G_EDGE_ID=r9ga&&r9gb?add(r9ga.id,r9gb.id,'street'):-1;globalThis.__R9G_GRID=[...nodes.values()].map(n=>({...n}));P.nodes=nodes;return P;";
const variants = {
...
const result = {
  method: 'fresh actual-Chrome Metal disposable pages; baseline and one appended real road-owner edge; candidate repeated per seed',
  ...
  candidate: { grid: '-8,-11->-7,-11', type: 'street', rationale: 'R8y predicts one lot on both seeds, dry and below its zone-paint slope threshold; accepted-lot contact risk is measured rather than hidden' },
...
const run = async (variant, seed, repeat = false) => {
  ...
  try {
    ...
    await page.waitForFunction(() => window.__sim?.ready === true && Array.isArray(globalThis.__R9G_GRID), null, { timeout: 240000 });
    ...
    const state = await page.evaluate(async ({ variant, seed }) => {
      ...
      const grid = globalThis.__R9G_GRID, a = grid.find(n => n.i === -8 && n.j === -11), b = grid.find(n => n.i === -7 && n.j === -11);
      const edgeId = globalThis.__R9G_EDGE_ID;
    ...
    if (!repeat) {
      ...
      if (state.midpoint) {
        await page.evaluate(({ x, z }) => {
          const s = window.__sim, y = s.world.terrain.getHeight(x, z);
          s.setCamera({ target: [x, y + 5, z], yaw: 0.72, pitch: 0.78, distance: 250 });
        }, state.midpoint);
...
try {
  for (const seed of [1337, 7]) {
    result.runs.push(await run('baseline', seed));
    result.runs.push(await run('candidate', seed));
    result.runs.push(await run('candidate', seed, true));
  }
  for (const seed of [1337, 7]) {
