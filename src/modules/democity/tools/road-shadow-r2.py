...
r=Path('shots/democity/rdev2');a=np.asarray(Image.open(r/'roadmask-on.png').convert('RGB'),float);b=np.asarray(Image.open(r/'roadmask-off.png').convert('RGB'),float);L=lambda x:x@np.array([.2126,.7152,.0722]);l=L(a);mask=abs(l-L(b))>6
for i in range(3):
