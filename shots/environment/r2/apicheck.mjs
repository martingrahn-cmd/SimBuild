// Critic API-contract probe for the environment module, round 2 (throwaway). Run: node shots/environment/r2/apicheck.mjs
...
const probe = async () => page.evaluate(async () => {
  ...
  const snap = () => ({
    ...
    envIntensity: +s.engine.scene.environmentIntensity.toFixed(3),
    fogDensity: w.fogDensity, night: +w.night.toFixed(3), moonPhase: +(w.moonPhase ?? NaN).toFixed(3),
    fogColor: s.engine.scene.fog?.color.getHexString(),
  ...
  await waitFrames(1);
  out.hours['12'] = snap();
  for (const h of [6.5, 17.5, 22, 0, 3, 5.9, 18.1, 19, 24]) { s.setTime(h); await waitFrames(14); out.hours[String(h)] = snap(); }
  s.setTime(12); await waitFrames(14);
  ...
  if (apiObj && apiObj.setWeather) {
    ...
    apiObj.setWeather('rain'); await waitFrames(4);
    const rain = { cloud: w.cloudiness, rain: w.rain, fog: w.fogDensity, preset: w.preset, wetness: +w.wetness.toFixed(3), sceneFog: scene.fog.density, sunIntensity: +w.sunIntensity.toFixed(3), exposure: +w.exposure.toFixed(3), evt };
    apiObj.setWeather('fog'); await waitFrames(4);
    const fog = { cloud: w.cloudiness, rain: w.rain, fog: w.fogDensity, preset: w.preset, sceneFog: scene.fog.density, sunIntensity: +w.sunIntensity.toFixed(3), exposure: +w.exposure.toFixed(3) };
    apiObj.setWeather('bogus'); await waitFrames(1);
    const bogus = { preset: w.preset, cloud: w.cloudiness };
    apiObj.setWeather({ cloudiness: 5, rain: -1, fogDensity: -3 }); await waitFrames(1);
    const clamp = { cloud: w.cloudiness, rain: w.rain, fog: w.fogDensity, preset: w.preset };
    apiObj.setWeather('partly'); await waitFrames(2);
  ...
  out.triangles = s.stats().triangles;
  return out;
});
...
const d1 = await page.evaluate(() => JSON.stringify([window.__sim.world.weather.sunDir.toArray(), window.__sim.world.weather.sunIntensity, window.__sim.engine.renderer.toneMappingExposure, [window.__sim.world.weather.skyLight.r, window.__sim.world.weather.skyLight.g, window.__sim.world.weather.skyLight.b]]));
await open('http://127.0.0.1:5173/?showcase=environment&headless=1&time=6.5');
const d2 = await page.evaluate(() => JSON.stringify([window.__sim.world.weather.sunDir.toArray(), window.__sim.world.weather.sunIntensity, window.__sim.engine.renderer.toneMappingExposure, [window.__sim.world.weather.skyLight.r, window.__sim.world.weather.skyLight.g, window.__sim.world.weather.skyLight.b]]));
res.deterministic = d1 === d2; res.detSample = d1;
res.consoleErrors = consoleErrors;
fs.writeFileSync('shots/environment/r2/apicheck.out.json', JSON.stringify(res, null, 2));
